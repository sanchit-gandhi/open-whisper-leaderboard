...
def main(args) -> None:
    ...
    model = whisperx.load_model(args.model_id, device="cuda", device_index=args.device, language="en", compute_type="float16")
    ...
    def benchmark(batch):
        ...
        batch["predictions"] = data_utils.normalizer("".join([segment["text"] for segment in result["segments"]])).strip()
